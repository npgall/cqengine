...
public class AttributeBytecodeGenerator {
    ...
    /**
     * Method name prefixes constituting Getters
     */
    private static final String[] GETTER_PREFIXES = {"get", "is", "has"};

    /**
     * Auto-generates and instantiates a set of attributes which read values from the Getters in the given POJO class.
     * <p>
     * Attributes will be generated for methods with specific prefixes (get, is, has) which are non-private and parameterless, 
     * declared directly in the POJO class or inherited, as long as the access modifiers on inherited methods in their superclass(es) 
     * allow those methods to be accessed from the package of the POJO class. So if the POJO class is in the same package as a 
     * superclass, then attributes will be generated for package-private, protected, and public methods in the superclass. If the
     * POJO class is in a different package from the superclass, then attributes will be generated for protected and
     * public methods in the superclass.
     * Methods decorated with the {@link AttributeGenerationExclude} annotation are excluded.
     *
     * @param pojoClass The POJO class containing methods for which attributes are to be created
     * @return A map of field/attribute names to Attribute objects which read values from the methods in the given POJO
     * class
     */
    public static <O> Map<String, ? extends Attribute<O, ?>> createAttributesForGetters(Class<O> pojoClass) {
        final Map<String, Attribute<O, ?>> attributes = new TreeMap<String, Attribute<O, ?>>();
        Class currentClass = pojoClass;
        while (currentClass != null && currentClass != Object.class) {
            for (Method method : currentClass.getDeclaredMethods()) {
                try {
                    ...
                    if (!Modifier.isStatic(modifiers) && !Modifier.isPrivate(modifiers)) {
                        ...
                        // Only getters (method name starts with one of the prefixes)
                        String methodName = method.getName();
                        String attributeName = null;
                        for (int i = 0; i < GETTER_PREFIXES.length; i++) {
                            if (methodName.startsWith(GETTER_PREFIXES[i])) {
                                // Remove the prefix from the attribute name
                                attributeName = methodName.substring(GETTER_PREFIXES[i].length());
                                break;
                            }
                        }
                        if (attributeName == null) {
                            ...
                            // Lower-case first letter of the attribute name (so that the attribute name for getValue becomes value)
                            attributeName = attributeName.substring(0, 1).toLowerCase() + attributeName.substring(1);
                        }

                        // Exclude
                        if (method.isAnnotationPresent(AttributeGenerationExclude.class)) {
                            continue;
                        }
                        ...
                        // only generate an attribute for it, if the field is actually accessible from the subclass,
                        // that is it is declared public or protected...
                        if (!method.getDeclaringClass().getPackage().getName().equals(pojoClass.getPackage().getName())
                        ...
                        if (methodReturnType.isPrimitive()) {
                            Class<?> wrapperType = getWrapperForPrimitive(methodReturnType);
                            attributeClass = generateSimpleAttributeForGetter(pojoClass, wrapperType, methodName, attributeName);
                        } else if (Iterable.class.isAssignableFrom(methodReturnType)) {
                            ...
                            Class<?> genericType = (Class<?>) actualTypeArguments[0];
                            attributeClass = generateMultiValueNullableAttributeForGetter(pojoClass, genericType, methodName, true, attributeName);
                        } else if (methodReturnType.isArray()) {
                            Class<?> componentType = methodReturnType.getComponentType();
                            if (componentType.isPrimitive()) {
                                Class<?> wrapperType = getWrapperForPrimitive(componentType);
                                attributeClass = generateMultiValueNullableAttributeForGetter(pojoClass, wrapperType, methodName, false, attributeName);
                            }
                            else {
                                attributeClass = generateMultiValueNullableAttributeForGetter(pojoClass, componentType, methodName, true, attributeName);
                            }
                        } else {
                            attributeClass = generateSimpleNullableAttributeForGetter(pojoClass, methodReturnType, methodName, attributeName);
                        }
                        Attribute<O, ?> attribute = attributeClass.newInstance();
                        attributes.put(attribute.getAttributeName(), attribute);
                    }
                } catch (Throwable e) {
                    throw new IllegalStateException("Failed to create attribute for method: " + method.toGenericString(), e);
                }
